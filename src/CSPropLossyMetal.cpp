...
CSPropLossyMetal::CSPropLossyMetal(ParameterSet* paraSet) : CSPropMetal(paraSet) {Type=(CSProperties::PropertyType)(LOSSY_METAL | METAL);Init();}
CSPropLossyMetal::CSPropLossyMetal(CSPropLossyMetal* prop, bool copyPrim) : CSPropMetal(prop, copyPrim)
{
	Type=(CSProperties::PropertyType)(LOSSY_METAL | METAL);
	Init();
	Conductivity.Copy(&prop->Conductivity);
}
CSPropLossyMetal::CSPropLossyMetal(unsigned int ID, ParameterSet* paraSet) : CSPropMetal(ID,paraSet) {Type=(CSProperties::PropertyType)(LOSSY_METAL | METAL);Init();}
